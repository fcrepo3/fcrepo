...
// @javax.jws.WebService(serviceName = "Fedorg.apache.axis.utils.JavaUtils isAttachmentSupportedora-API-M-MTOM-Service", portName =
// "Fedora-API-M-MTOM-Port-SOAPHTTP", targetNamespace =
// "http://fedora-commons.org/2011/07/definitions/api/", wsdlLocation =
// "../resources/wsdl/Fedora-API-M-MTOM.wsdl", endpointInterface =
// "org.fcrepo.server.management2.FedoraAPIMMTOM")
@MTOM
...
public class FedoraAPIMMTOMImpl implements FedoraAPIMMTOM {
	 ...
	 */
	public String compareDatastreamChecksum(String pid, String dsID,
			String versionDate) {
		assertInitialized();
		try {
			MessageContext ctx = context.getMessageContext();
			return s_management.compareDatastreamChecksum(
					ReadOnlyContext.getSoapContext(ctx), pid, dsID,
					DateUtility.parseDateOrNull(versionDate));
		} catch (Throwable th) {
	 ...
	 * datatype )*
	 */
	public boolean purgeRelationship(String pid, String relationship,
		...
		try {
			...
			return s_management.purgeRelationship(
					ReadOnlyContext.getSoapContext(ctx), pid, relationship,
	...
	public String purgeObject(String pid, String logMessage, boolean force) {
		...
		try {
			...
			return DateUtility.convertDateToString(s_management.purgeObject(
					ReadOnlyContext.getSoapContext(ctx), pid, logMessage));
		} catch (Throwable th) {
	 ...
	 * String pid ,)String dsID ,)org.fcrepo.server.types2.gen.ArrayOfString
	...
	public String modifyDatastreamByValue(String pid, String dsID,
			...
			String mimeType, String formatURI, DataHandler dsContent,
	 ...
	 */
	public String setDatastreamState(String pid, String dsID, String dsState,
			String logMessage) {
	 ...
	 */
	public String setDatastreamVersionable(String pid, String dsID,
	 ...
	 * pid ,)String dsID ,)org.fcrepo.server.types2.gen.ArrayOfString altIDs
	...
	public String addDatastream(String pid, String dsID,
		...
		try {
			MessageContext ctx = context.getMessageContext();
			String[] altIDsArray = altIDs.getItem().toArray(new String[0]);
			return s_management.addDatastream(
					ReadOnlyContext.getSoapContext(ctx), pid, dsID,
					altIDsArray, dsLabel, versionable, mimeType, formatURI,
					dsLocation, controlGroup, dsState, checksumType, checksum,
					logMessage);
		} catch (Throwable th) {
		...
		try {
			...
			return DateUtility.convertDateToString(s_management
					.modifyDatastreamByReference(
							ReadOnlyContext.getSoapContext(ctx), pid, dsID,
							altIDsArray, dsLabel, mimeType, formatURI,
							dsLocation, checksumType, checksum, logMessage,
							null));
		} catch (Throwable th) {
	 ...
	 * ,)String datatype )*
	 */
	public boolean addRelationship(String pid, String relationship,
			String object, boolean isLiteral, String datatype) {
		LOG.debug("start: addRelationship");
		assertInitialized();
		try {
			MessageContext ctx = context.getMessageContext();
			return s_management.addRelationship(
					ReadOnlyContext.getSoapContext(ctx), pid, relationship,
					object, isLiteral, datatype);
		} catch (Throwable th) {
	...
	public List<String> getNextPID(java.math.BigInteger numPIDs,
		...
		try {
			...
			String[] aux = s_management.getNextPID(
					ReadOnlyContext.getSoapContext(ctx), numPIDs.intValue(),
					pidNamespace);
	...
	public DataHandler getObjectXML(String pid) {
		...
		try {
			MessageContext ctx = context.getMessageContext();
			InputStream in = s_management.getObjectXML(
	...
	public String modifyObject(String pid, String state, String label,
			String ownerId, String logMessage) {
		LOG.debug("start: modifyObject, " + pid);
		assertInitialized();
		try {
			MessageContext ctx = context.getMessageContext();
			return DateUtility.convertDateToString(s_management.modifyObject(
					ReadOnlyContext.getSoapContext(ctx), pid, state, label,
					ownerId, logMessage, null));
		} catch (Throwable th) {
	 ...
	 */
	public List<String> purgeDatastream(String pid, String dsID,
			String startDT, String endDT, String logMessage, boolean force) {
		LOG.debug("start: purgeDatastream, " + pid + ", " + dsID);
		assertInitialized();
		try {
			MessageContext ctx = context.getMessageContext();
			return toStringList(s_management.purgeDatastream(
					ReadOnlyContext.getSoapContext(ctx), pid, dsID,
					DateUtility.parseDateOrNull(startDT),
					DateUtility.parseDateOrNull(endDT), logMessage));
		} catch (Throwable th) {
	...
	public List<org.fcrepo.server.types2.gen.Datastream> getDatastreamHistory(
		...
		assertInitialized();
		try {
			org.fcrepo.server.storage.types.Datastream[] intDatastreams = s_management
	...
	public List<org.fcrepo.server.types2.gen.Datastream> getDatastreams(
		...
		try {
			MessageContext ctx = context.getMessageContext();
			org.fcrepo.server.storage.types.Datastream[] intDatastreams = s_management
					.getDatastreams(ReadOnlyContext.getSoapContext(ctx), pid,
							DateUtility.parseDateOrNull(asOfDateTime), dsState);
			return getGenDatastreams(intDatastreams);
		} catch (Throwable th) {
	 ...
	 */
	public org.fcrepo.server.types2.gen.Datastream getDatastream(String pid,
