...
public class TestObjectLastModDate extends FedoraServerTestCase {
	...
	public void testFCREPO238() throws Exception{
		String pid=apim.ingest(IOUtils.toByteArray(new FileInputStream(RESOURCEBASE + "/" + FOXMLPATH)), FOXML1_1.uri, "testing fcrepo 238");
		...
		for (DatastreamDef dd:apia.listDatastreams(pid, null)){
			...
			Assert.assertTrue("object last modificaton date is before datastream's create date. check FCREPO-238",objDate.compareTo(dsDate) > -1);
		}
